...
async function expectLightReadablePage(page: Page) {
  ...
  expect(Math.min(...result.background)).toBeGreaterThan(225);
  expect(Math.max(...result.foreground)).toBeLessThan(100);
}
...
test("major learner interfaces render on the v87.3 light system without page crashes", async ({
  page,
}, testInfo) => {
  ...
  for (const route of majorRoutes) {
    await test.step(route.path, async () => {
      ...
      await expectLightReadablePage(page);
    });
  }
