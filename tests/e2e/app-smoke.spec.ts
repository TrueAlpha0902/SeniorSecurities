...
async function expectDarkSurface(page: Page, selector: string) {
  ...
  expect(
    backgrounds.length,
    `Expected at least one dark surface for ${selector}`,
  ).toBeGreaterThan(0);
  ...
  for (const background of backgrounds) {
    ...
    expect(
      Math.max(...channels),
      `${selector} must use a dark surface, got ${background}`,
    ).toBeLessThan(72);
  }
}
...
test("catalog, calculator and five subject plans render without a blank screen", async ({
  ...
  await expectDarkSurface(page, ".v90-app-stage");
  ...
  await expectDarkSurface(page, ".settings-section-panel");
  await expectDarkSurface(page, ".settings-section-panel > header");
...
test("securities home presents exactly three official subjects", async ({ page }) => {
  ...
  await expectDarkSurface(page, ".v90-paper-card");
  await expectDarkSurface(page, ".v90-home-section");
  await expectDarkSurface(page, ".v90-path-content");
...
test("securities general practice is untimed, text-only and reveals a neutral explanation", async ({
  ...
  await expectDarkSurface(page, ".image-quiz-controls");
  await expectDarkSurface(page, ".unified-explanation-surface");
...
test("foreign-exchange shows all sessions, custom random count and untimed general practice", async ({
  ...
  await expectDarkSurface(page, ".fx-quiz-actions");
  await expectDarkSurface(page, ".fx-explanation");
