...
test("app boots, opens calculator and settings without a blank screen", async ({ page }, testInfo) => {
  await page.goto("/auth");
  await expect(page.locator("#root")).not.toBeEmpty();
  await expect(page.getByRole("heading", { name: /登入|會員|尚未設定 Supabase/ }).first()).toBeVisible();
...
test("trial quiz restores visible correct and wrong answer states", async ({ page }, testInfo) => {
  await page.goto("/trial");
  await expect(page.locator("#root")).not.toBeEmpty();
  await expect(page.getByRole("button", { name: /選擇 \(1\)/ })).toBeVisible({ timeout: 20_000 });
  ...
  await expect(page.locator(".glass-answer-wrong")).toHaveCSS("color", "rgb(255, 255, 255)");
  await expect(page.locator(".glass-answer-correct")).toHaveCSS("color", "rgb(255, 255, 255)");
  await page.screenshot({ path: testInfo.outputPath("trial-answer-colors.png"), fullPage: true });
});
