...
async function expectLightReadableRoot(page: Page) {
  ...
  expect(Math.min(...background)).toBeGreaterThan(225);
  expect(Math.max(...text)).toBeLessThan(100);
}
...
test("v87.3 light theme is the default and no dead notification button remains", async ({
  page,
}) => {
  await page.goto("/securities");
  await expect(page.locator(".theme-v93")).toBeVisible({ timeout: 20_000 });
  await expectLightReadableRoot(page);
