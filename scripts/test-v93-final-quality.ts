...
for (const token of [
  ...
  assert(theme.includes(token), `Final dark/accessibility theme is missing ${token}.`);
}
assert(
  [
    ".floating-calculator",
  ...
  "Dark surface coverage must include calculator, administration, search, settings, and account interfaces.",
);
assert(
...
assert(
  indexHtml.includes('<meta name="theme-color" content="#07090c" />')
    && indexHtml.includes('<meta name="color-scheme" content="dark" />')
    && indexHtml.includes('background:#07090c;color:#f4f7fa')
    && indexHtml.includes('apple-mobile-web-app-status-bar-style" content="black-translucent"'),
  "The initial browser shell must be dark before React mounts.",
);
assert(
  viteConfig.includes('theme_color: "#07090c"')
    && viteConfig.includes('background_color: "#07090c"'),
  "The PWA manifest and splash screen must use the v93 dark background.",
);
...
console.log(
  `v93 final quality contracts passed: ${buttonCount} buttons, ${formControlCount} form controls, `
