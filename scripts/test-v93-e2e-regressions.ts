...
assert(
  themeV93.includes("color-scheme: light")
    && themeV93.includes("--v93-bg: #f4f7fb")
    && themeV93.includes("--v93-surface: #ffffff")
    && themeV93.includes("--v93-accent: #2b6ea6")
  ...
  "The active v87.3 light theme must outrank legacy paper selectors on major app surfaces.",
);

assert(
  appSmoke.includes('expectLightSurface(page, ".v90-app-stage")')
    && appSmoke.includes('expectLightSurface(page, ".settings-section-panel")')
    && appSmoke.includes('expectLightSurface(page, ".v90-path-content")')
    && !appSmoke.includes("expectDarkSurface("),
  "Browser smoke tests must guard the actual v87.3 light surfaces seen by learners.",
);

assert(
  appSmoke.includes("evaluateAll((elements)")
    && appSmoke.match(/\.answer-result-label"\)\.first\(\)/g)?.length === 2,
  "Browser smoke tests must support repeated surfaces and intentional answer badges.",
);
...
assert(
  ...
  "Catalog status, metadata and progress labels must keep accessible active-theme contrast.",
);
...
console.log(
  "v93 E2E regression contracts passed: v87.3 light surfaces, scoped tokens, accessible catalog progress, repeated surfaces, answer feedback, mock availability, responsive utilities, and browser selectors are protected.",
);
