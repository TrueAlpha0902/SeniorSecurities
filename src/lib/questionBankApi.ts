...
const localPreviewEnabled = Boolean(
  RUNTIME_ENV?.DEV && RUNTIME_ENV.VITE_LOCAL_PREVIEW_ACCESS === "1",
);

const localPreviewAccess: QuestionBankAccess = {
  token: "local-preview-token",
  account: "local-preview-user",
};
...
async function readSession(forceRefresh: boolean): Promise<QuestionBankAccess> {
  if (localPreviewEnabled) return localPreviewAccess;
