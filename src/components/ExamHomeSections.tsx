...
import { HandwrittenAsset, HandwrittenIcon, HandwrittenLabel } from "./HandwrittenAsset";
import {
  SketchExamInfoArt,
  SketchLearningChart,
...
function ExamHeroArtwork({ tone }: { tone: ExamThemeTone }) {
  return (
    <HandwrittenAsset
      category="icons"
      name={tone === "foreign-exchange" ? "fx-international" : "securities"}
      text={tone === "foreign-exchange" ? "外匯學習插圖" : "證券學習插圖"}
      className="v91-hero-brand-art"
      decorative
      loading="eager"
    />
  );
    ...
    <header className={`premium-home-hero v90-home-hero is-${tone}`}>
      <section className="v90-hero-banner" aria-label={`${title}學習概況`}>
        <div className="v90-home-copy">
          <div className="v90-title-row">
            <h1><HandwrittenLabel name={tone === "foreign-exchange" ? "foreign-exchange" : "securities"} text={title} className="v91-home-title-image" /></h1>
            <p>{eyebrow}</p>
          </div>
          <span>{subtitle}</span>
        </div>
      ...
      <section className="v90-overview-grid" aria-label="進度與考試計畫">
        <article className="v90-paper-card v90-progress-card">
          <h2><HandwrittenLabel name="overall-progress" text="整體進度" className="v91-section-label" /></h2>
        ...
        >
          ...
          <span className="v90-plan-copy">
            <small><HandwrittenLabel name="exam-date" text="共同考試日期" className="v91-inline-label" /></small>
        ...
        <Link className="v90-paper-card v90-daily-card" to={dailyActionTo}>
          <span>
            <small><HandwrittenLabel name="today-goal" text="今日建議題數" className="v91-inline-label" /></small>
            <strong>{Math.max(0, dailyCount)}<em>題</em></strong>
          </span>
          <span className="v90-outline-action">
            <HandwrittenLabel name="start-practice" text={dailyActionLabel} className="v91-button-label" /><ArrowRight aria-hidden="true" size={18} />
          </span>
        </Link>
      </section>
    </header>
  ...
  return (
    <section className="premium-home-section v90-home-section v90-quick-section" aria-label={title}>
      <div className="v90-section-heading">
        <h2>{title === "學習捷徑" ? <HandwrittenLabel name="practice" text={title} className="v91-section-label" /> : title}</h2>
      </div>
      <div className={`v90-quick-grid is-${actions.length}-items`}>
        {actions.map((action) => {
          ...
          const content = (
            <>
              <span className="v90-quick-icon v91-quick-icon">
                <HandwrittenIcon name={quickActionIconAsset(action.label)} text={action.label} />
                <Icon aria-hidden="true" size={22} className="v91-icon-fallback" />
              </span>
              <span><strong>{quickActionLabelAsset(action.label) ? <HandwrittenLabel name={quickActionLabelAsset(action.label)!} text={action.label} className="v91-quick-label" /> : action.label}</strong><small>{action.description}</small></span>
            </>
          );
          return action.to
...
export function ExamSubjectPath({
  ...
  return (
    <section id="learning-path" className="premium-home-section v90-home-section v90-subject-section" aria-labelledby="v90-path-title">
      <div className="v90-section-heading">
        <h2 id="v90-path-title"><HandwrittenLabel name="learning-progress" text={title} className="v91-section-label" /></h2>
      </div>
          ...
          <li key={item.id}>
            ...
            <Link className="v90-path-content" to={item.to}>
              <span className="v90-path-copy">
                <strong>{subjectLabelAsset(item.title) ? <HandwrittenLabel name={subjectLabelAsset(item.title)!} text={item.title} className="v91-subject-label" /> : item.title}</strong>
    ...
    <section id="learning-summary" className={`v90-learning-summary is-${tone}`} aria-label="學習數據與考試資訊">
      <article className="v90-learning-data-card">
        <div className="v90-section-heading is-inline">
          <h2><HandwrittenLabel name="learning-summary" text="學習數據摘要" className="v91-section-label" /></h2>
          <span>近 7 天</span>
        </div>
      ...
      <article className="v90-exam-info-card">
        <div className="v90-section-heading is-inline"><h2><HandwrittenLabel name="exam-date" text="考試資訊" className="v91-section-label" /></h2></div>
        <dl>
...
function quickActionLabelAsset(label: string): string | null {
  const map: Record<string, string> = {
    ...
    "模擬考": "mock",
    "錯題練習": "wrong-review",
    "收藏練習": "favorite-questions",
    "弱點分析": "learning-summary",
  };
  return map[label] ?? null;
}

function quickActionIconAsset(label: string): string {
  const map: Record<string, string> = {
    "隨機練習": "practice",
    "章節練習": "bank",
    "歷屆練習": "bank",
...
function subjectLabelAsset(title: string): string | null {
  const map: Record<string, string> = {
    "投資學": "investment",
    "財務分析": "financial-analysis",
    "證券相關法規與實務": "laws-practice",
    "國外匯兌業務": "remittance",
    "進出口外匯業務": "trade",
  };
