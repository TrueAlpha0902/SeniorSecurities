...
export function SimilarQuestionsPage() {
  ...
  async function revealGroup(group: ResolvedSimilarGroup): Promise<void> {
    ...
    if (answeredQuestions.length !== group.questions.length) {
      const message = "請先完成兩題作答，再查看解析。";
      setActionError(message);
      announceInteractionFeedback(message, "warning", 3200);
      return;
    }
    ...
    try {
      ...
      const message = "兩題作答已儲存，解析已開啟。";
      setActionMessage(message);
      announceInteractionFeedback(message, "success");
    } catch (reason) {
      const message = reason instanceof Error
        ? reason.message
        : "無法儲存這次相似題作答，請稍後再試。";
      setActionError(message);
      announceInteractionFeedback(message, "error", 4400);
    } finally {
  ...
  function resetGroup(group: ResolvedSimilarGroup): void {
    setActionError(null);
    setActionMessage("已清除本組作答，可重新比較兩題。");
    announceInteractionFeedback("已清除本組作答。", "info");
    setAnswers((current) => {
      const next = { ...current };
  ...
  return (
    <div className="page-stack similar-learning-page">
      {actionError ? <V93InlineNotice tone="error">{actionError}</V93InlineNotice> : null}
      {actionMessage ? <V93InlineNotice tone="success">{actionMessage}</V93InlineNotice> : null}
      ...
      <GlassCard className="similar-learning-toolbar" as="section">
        <div className="similar-filter" aria-label="科目篩選">
          <button
            ...
            aria-pressed={selectedBankId === "all"}
          ...
          {bankOptions.map((option) => (
            <button
              ...
              aria-pressed={selectedBankId === option.bankId}
...
function SimilarPairCard({
  ...
  return (
    <GlassCard
      className="similar-pair-card"
      as="article"
      tabIndex={-1}
      aria-busy={savingAttempt || undefined}
    >
