...
- 將抽屜與確認視窗改為共用參考計數式 body scroll lock；內層視窗關閉時維持外層鎖定，最後一層關閉才精確恢復原始 overflow。
- 權限操作成功後立即關閉確認視窗，再平行更新會員列表與明細，避免背景同步期間遮罩持續阻擋操作。
- 新增 runtime 契約與 Playwright 回歸測試，驗證雙層鎖逐層釋放、原始 overflow 恢復及頁面可重新捲動。
- 不修改 Supabase schema、會員權限、啟用碼或題庫資料。

## 2026-08-26 — 題庫專屬啟用碼與後台分流

- 確認既有啟用碼資料已綁定單一 `exam_id`；指定的既有碼維持證券高業專用，正式明碼不寫入版本庫。
- 新增唯一名稱的題庫專屬兌換 RPC，原子比對啟用碼與目前題庫後才增加使用次數，避免在錯誤題庫頁消耗另一題庫的碼。
- 前端兌換呼叫改為明確傳送目前 `examId`，並在輸入頁說明該頁只接受對應題庫的啟用碼。
- 修復登入後遺失外匯 scope、被導回預設高業啟用頁的問題。
- 後台生成器以兩個明確選項分開建立證券高業／初階外匯啟用碼，按鈕與一次性結果固定標示適用題庫。
- 後台 API 缺少或收到非法 `examId` 時回傳 400，不再靜默默認證券高業。
- 新增題庫 scope 契約測試與 Supabase migration；保留舊兌換 RPC 供舊 PWA 相容。
- 正式資料庫已套用 migration，並以完整回滾交易驗證高業、外匯、雙向跨題庫拒絕、無效題庫與 RPC 權限，沒有留下測試資料。

## 2026-08-26 — 啟用碼改為無連字號顯示

- 新建立的啟用碼改為回傳、顯示及複製純英數格式，不再自動每四碼插入 `-`。
- 建立與兌換輸入框改用無連字號範例；不在原始碼中寫入任何正式啟用碼。
- 保留既有正規化規則，因此舊有含 `-` 的啟用碼與新的無連字號輸入仍會命中同一筆雜湊。
- 新增格式契約，驗證大小寫、空白、舊式連字號及 SHA-256 雜湊相容性。
- 不修改 Supabase schema、既有啟用碼、使用次數或會員權限。

## 2026-08-26 — 管理後台權限確認視窗修復

- 修復「開通證券高業」與「開通初階外匯」共用確認視窗在 portal 後遺失樣式、落到可視範圍外的問題。
- 將必要的 backdrop、dialog、桌面／手機與 reduced-motion 規則移至現行載入的 `theme-current.css`，不重新啟用歷史主題檔。
- 擴充管理後台契約，鎖定兩種題庫權限的 `examId` 傳遞及 `/api/admin/action` POST 流程。
- 新增 Playwright 桌面與手機回歸案例，驗證確認視窗完整覆蓋 viewport 且「確認執行」保持可見，並透過真實共用 React 元件驗證開啟與取消流程。
- 不修改 Supabase schema、migration、題庫內容或現有會員權限。
