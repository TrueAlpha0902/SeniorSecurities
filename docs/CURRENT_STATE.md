...
- 已重現會員明細抽屜與權限確認視窗各自保存／恢復 `body.style.overflow`，導致巢狀視窗關閉後頁面仍殘留 `overflow: hidden` 的問題。
- 新增共用、參考計數式 `useBodyScrollLock`；確認視窗關閉後仍保留抽屜所需的鎖定，關閉最後一層視窗後才恢復開啟前的原始值。
- 權限操作收到成功回應後先關閉確認視窗，會員列表與明細改為平行背景更新，不再讓資料同步時間阻擋抽屜捲動。
- runtime 契約及 Playwright 瀏覽器測試已驗證 `hidden → hidden → hidden → 原始值` 的完整生命週期，最後可用滑鼠滾輪再次捲動。
- 此修復僅影響前端視窗生命週期，不變更 Supabase schema、啟用碼、既有會員權限或題庫內容。
