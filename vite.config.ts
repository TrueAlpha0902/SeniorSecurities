      ...
      const handler = async (
        ...
        try {
          ...
          if (resource === "securities") {
            const manifestPath = resolve(process.cwd(), "public", "data", "question-release-manifest.json");
            const manifest = JSON.parse(await readFile(manifestPath, "utf8")) as {
              banks: Array<{ bankId: string; chapters: Array<{ path: string }> }>;
            };
            const shards = await Promise.all(manifest.banks.flatMap((bank) => bank.chapters).map(async (chapter) => {
              const source = await readFile(resolve(process.cwd(), "public", chapter.path), "utf8");
              return JSON.parse(source) as { chapter: { questions: Array<Record<string, unknown>> } };
            }));
            const allSecuritiesQuestions = shards.flatMap((shard) => shard.chapter.questions);
          ...
          for (let currentSession = 23; currentSession <= 47; currentSession += 1) {
            for (const currentSubject of ["remittance", "trade"]) {
              const path = resolve(process.cwd(), "api", "_data", "foreign-exchange", `${currentSession}-${currentSubject}.json`);
              const shard = JSON.parse(await readFile(path, "utf8")) as PreviewQuestion[];
